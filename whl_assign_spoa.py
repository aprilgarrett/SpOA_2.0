...
#make empty array
go_out = np.empty(shape=(54427,2), dtype = object)
spu_out = np.empty(shape=(54427,2), dtype = object)

i=0 # start counter

# in col 3 of whl-annot, there are gene names. add these
with open('/users/a/m/amakukho/RESEARCH/SpOA/combined_data/analysis/cmh.all.id') as master_file:
...
np.savetxt('/users/a/m/amakukho/RESEARCH/SpOA/combined_data/analysis/cmh.all.GO', go_out,fmt='%5s', delimiter='\t')
...
# pull out snp ID
i=0
snp_out = np.empty(shape=(54427,62), dtype = object)

with open('/users/a/m/amakukho/RESEARCH/SpOA/combined_data/analysis/cmh.master.sort.out') as master_file:
...
np.savetxt('/users/a/m/amakukho/RESEARCH/SpOA/combined_data/analysis/cmh.master_goodnm.out', out_final,fmt='%5s', delimiter='\t')